...
export default class GoodsDetail extends Component{
    ...
    addcart(){
      ...
      var tempString = this.state.item.commodityInfo+" "+goodId+" "+storeId+" "+storeName+" "+this.state.item.commodityCoverPicUrl+" "+this.state.item.price;
      DeviceStorage.get('cart').then((tel)=>{
        if(tel == null || tel == ''){
          DeviceStorage.save("cart",tempString);
        } else {
          tempdata = tel.split('\n');
            var flag = 0;
            for(var i=0;i<tempdata.length;i++)
            if(tempdata[i]==tempString){flag=1;break;}
            if(flag==0)
            tel = tel+'\n'+tempString;
            DeviceStorage.save('cart',tel);
           
        }
    })
    }
    deletecart(){
      DeviceStorage.delete("cart");
    }
    render(){
