...
export default class ShopCart1 extends Component {
    ...
    pay(){
        ...
        if(foreverdata.length == 1){
            console.log("Ok Here");
            if (foreverdata[0].shopItems.length==0) 
                foreverdata=[];
            }
            else {
                for (var i = 0; i < foreverdata.length; i++) {
            ...
            this.setState({data:data,
                status:tempStatusArr,
                totalPrice:0.00
            });
            this.props.navigation.navigate('OrderOk',
                {   shopId:storeId,
                    shopName:shopName,
          ...
          <View style={styles.cellStyle}>
              <TouchableOpacity onPress={() => this.checkItem(sectionIndex, index)}>
                  <Image style={styles.checkBox} 
                      source={statusItem.checked ? require('./assets/ic_selected.png') : require('./assets/ic_defult.png')} 
                      resizeMode={'center'}
                  />
              </TouchableOpacity>
              ...
              />
              <View style={{justifyContent: commonStyle.around, 
                  flex: 1, 
                  marginHorizontal: 10, 
                  height: 50}}
              >
        ...
        return (
            <View style={styles.sectionHeader}>
                <TouchableOpacity onPress={() => this.checkedShop(index)}>
                    <Image style={styles.checkBox} 
                        source={shop.checked ? require('./assets/ic_selected.png') : require('./assets/ic_defult.png')} 
                        resizeMode={'center'}
                    />
                </TouchableOpacity>
                <Text style={{color: commonStyle.gray, 
                    fontSize: 12}}>
                    {section}
                </Text>
            </View>)
            ...
            <View style={styles.toolBar}>
                ...
                >
                    <TouchableOpacity onPress={() => this.checkAllShop()}>
                        <Image style={styles.checkBox} 
                            source={this.state.isSelectedAllItem ? require('./assets/ic_selected.png') : require('./assets/ic_defult.png')} 
                            resizeMode={'center'}
                        />
                    </TouchableOpacity>
                    <Text>全选</Text>
                ...
                <Overlay isVisible={this.state.isVisible}>
                    ...
                    <Input onChangeText={(value) => this.setState({tarPeople: value})} 
                        value={this.state.tarPeople}
                        placeholder='收件人姓名'
                        leftIcon={
                            <Image
                    ...
                    />
                    <Input onChangeText={(value) => this.setState({tarPhone: value})}
                        value={this.state.tarPhone}
                        placeholder='收件人手机'
                        leftIcon={
                            <Image
                    ...
                    />
                    <Input onChangeText={(value) => this.setState({tarAddress: value})}
                        value={this.state.tarAddress}
                        placeholder='收件人住址'
                        leftIcon={
                            <Image
